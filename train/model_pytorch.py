...
ERROR_MSG = (
    ...
    f"This code has been tested on version 2.7 and 2.9, and it is known that NaN (Not a Number) issues occur in 2.5 (2.6-2.8 untested). "
    f"It is recommended to use version {MIN_VERSION} or later."
)
...
class TransformerBlock(torch.nn.Module):
    ...
    def add_reg_dict(self, reg_dict: Dict[str, List]):
        ...
        for name, param in self.ffn_linear1.named_parameters():
            if "weight" in name and "norm" not in name:
        ...
        for name, param in self.ffn_linear2.named_parameters():
            if "weight" in name and "norm" not in name:
                reg_dict["normal"].append(param)
            elif "bias" in name:
                reg_dict["noreg"].append(param)
...
def apply_rotary_emb(xq: torch.Tensor, xk: torch.Tensor, cos: torch.Tensor, sin: torch.Tensor):
    ...
    def rotate_every_two(x):
        ...
        # 2. Explicitly separate x0, x1
        # unbind performs a view-only operation without memory copies
        x0, x1 = x.unbind(dim=-1)
        
        # 3. Recombine as [-x1, x0]
        # Use stack to explicitly allocate new memory; most robust for ONNX
        # Avoids side-effect risks from flip + narrow + neg_
        x_rotated = torch.stack([-x1, x0], dim=-1)
        
        # 4. Flatten back to (..., Dim)
        return x_rotated.flatten(-2)

    # Adjust cos/sin shapes for broadcasting (1, Seq, 1, Dim)
    # Assume cos/sin are already (Seq, Dim) (after repeat_interleave)
    cos = cos.view(1, xq.shape[1], 1, xq.shape[-1])
    sin = sin.view(1, xq.shape[1], 1, xq.shape[-1])

    # Core formula: Output = x * cos + rotate(x) * sin
    # Keep vectorized multiplication; efficiency remains high
...
class ValueHead(torch.nn.Module):
    ...
    def forward(self, x, mask, mask_sum_hw, mask_sum:float, input_global, extra_outputs: Optional[ExtraOutputs]):
        ...
        out_ownership = self.conv_ownership(outv1)
        out_scoring = self.conv_scoring(outv1)
        out_futurepos = self.conv_futurepos(x)
        out_seki = self.conv_seki(x)
        if mask is not None:
            out_ownership = out_ownership * mask
            out_scoring = out_scoring * mask
            out_futurepos = out_futurepos * mask
            out_seki = out_seki * mask
        ...
        #print("outsmix_logweights",outsmix_logweights.shape)
